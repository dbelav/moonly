...
const AllNftContainer = () =>{
    ...
    const normalizeAllNftData = (data) =>{
        return data.map(item =>{
            return{
                ...
                highestGlobalOffer: (item.highestGlobalOffer / 1000000000).toFixed(2),
            }
        })
    }

    useEffect(() =>{
        getDataAsync(allCollectionsFetching,
            allCollectionsFetched,
            'https://stats-mainnet.magiceden.io/collection_stats/popular_collections/sol?limit=20&window=7d&compressionMode=regular',
            '',
            normalizeAllNftData
            )
    },[])
    ...
    const renderAllNft = (allNft) => {
        if(allNftLoading){
            return  <Skeleton className='topCollectionSkeleton' count={20} baseColor="#202020" highlightColor="#444" height={40}/>
        }
        return allNft.map((item, index) => {
            return <AllNftItem data={item} key={item.symbol} index={index + 1}/>
        })
    }
        ...
        <div className='allNftContainer'>
            <div className='allNftInnerContainer'>
                <div className='allNftInnerContainerInfo'>
                    ...
                    <div className='allNftInnerContainerInfoTotalVolume'>Total Vol</div>
