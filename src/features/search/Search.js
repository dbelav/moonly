...
const Search = () => {
    const { searchCollections } = useSelector(
        (state) => state.searchNftCollections
    )

    const { onChangeFormText } = useSelector(
        (state) => state.searchNftCollections
    )
        ...
        async function getData() {
            if (onChangeFormText.length !== 0) {
                const data = await request(
                    `${API_MAINNET_V2}/unifiedSearch/collection/${onChangeFormText}?chain=SOL`
                )
        ...
        <div className="search">
            <form className="serachForm">
                <input
                    ...
                    onBlur={() =>
                        setTimeout(() => dispatch(onFocusForm(false)), 100)
                    }
                />
            </form>
            {showModal}
